...
const EditTipoPropiedad = () => {
  ...
  useEffect(() => {
    // Cargar datos de todos los tipos de propiedad y seleccionar el necesario al montar el componente
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost/tipos_propiedad');
        const tipoPropiedad = response.data.data.find(tp => tp.id === parseInt(id));
        if (tipoPropiedad) {
          setNombre(tipoPropiedad.nombre);
        } else {
          setMensaje('Tipo de propiedad no encontrado');
        }
      } catch (error) {
        console.error('Error fetching tipos de propiedad:', error);
        setMensaje('Error al cargar los tipos de propiedad');
      }
    };
    fetchData();
  }, [id]);
